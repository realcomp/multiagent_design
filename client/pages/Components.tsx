...
const specimenColumns: DataTableColumn<SpecimenRow>[] = [
  ...
  { key: "status", header: "Статус", priority: "medium", render: (row) => <span className="font-mono text-[11px] text-ink-muted">{row.status}</span> },
  ...
  return (
    <div className="mx-auto max-w-[1480px] px-5 py-8 sm:px-8 lg:py-10">
      <div className="mb-8 max-w-2xl">
        <p className="mb-2 font-mono text-[10px] font-semibold uppercase tracking-[0.16em] text-brand">Design system / v1.0</p>
        <h1 className="text-[28px] font-semibold tracking-[-0.04em] text-ink">Основа интерфейса</h1>
        <p className="mt-2 text-sm leading-6 text-ink-muted">Общие токены и примитивы для Runs, Trace и Run detail. Спокойная плотность данных, явные состояния, машинные значения без визуального шума.</p>
      </div>

      <section className="mb-8" aria-labelledby="tokens-heading">
        <div className="mb-3 flex items-end justify-between gap-4">
          <div>
            <p className="section-kicker">01 / Tokens</p>
            <h2 id="tokens-heading" className="section-title">Токены</h2>
          </div>
          <span className="hidden font-mono text-[10px] text-ink-subtle sm:block">8px spacing scale · light theme</span>
        </div>
        <div className="grid gap-px overflow-hidden border border-line bg-line sm:grid-cols-2 lg:grid-cols-4">
          ...
          <div className="bg-surface p-4">
            ...
            <div className="mt-3 space-y-3">
              <div><p className="text-lg font-semibold tracking-tight text-ink">Prose / 18</p><p className="mt-0.5 text-[10px] text-ink-subtle">Inter · 600</p></div>
              <div><p className="font-mono text-sm text-ink">$0.0341 · 34 426 мс</p><p className="mt-0.5 text-[10px] text-ink-subtle">IBM Plex Mono · 500</p></div>
            </div>
          </div>
          <div className="bg-surface p-4">
            <p className="token-label">Ритм</p>
            <div className="mt-3 flex items-end gap-3">
              {[1, 2, 3, 4, 5].map((step) => <div key={step} className="flex flex-col items-center gap-1"><span className="block bg-brand/80" style={{ height: step * 8, width: 8 }} /><span className="font-mono text-[9px] text-ink-subtle">{step * 8}</span></div>)}
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="mb-8" aria-labelledby="primitives-heading">
        <div className="mb-3">
          <p className="section-kicker">02 / Primitives</p>
        ...
        <div className="grid gap-4 lg:grid-cols-2">
          ...
          <ShowcaseCard title="DataTable" description="Плотные строки, sticky header, приоритеты колонок и раскрытие.">
            <DataTable
              ...
              renderExpanded={() => <p className="text-xs leading-5 text-ink-muted">Раскрытая область для input/output роли или дополнительного контекста.</p>}
            />
            <div className="mt-3 grid gap-3 md:grid-cols-2">
              <div>
                <p className="mb-1.5 font-mono text-[9px] uppercase tracking-[0.1em] text-ink-subtle">loading</p>
                <DataTable columns={specimenColumns} rows={[]} getRowId={(row) => row.id} loading />
              </div>
              <div>
                <p className="mb-1.5 font-mono text-[9px] uppercase tracking-[0.1em] text-ink-subtle">empty</p>
      ...
      <div className="border-t border-line pt-5 text-xs text-ink-subtle">Эти компоненты не завязаны на данные Runs и готовы к использованию в следующих экранах.</div>
    </div>
  );
}

function TokenSwatch({ name, className }: { name: string; className: string }) {
  return <div className="flex items-center gap-2"><span className={`size-3 rounded-[3px] ${className}`} /><span className="font-mono text-[11px] text-ink-muted">{name}</span></div>;
}

function ShowcaseCard({ title, description, children }: { title: string; description: string; children: React.ReactNode }) {
  return <div className="border border-line bg-surface p-4 sm:p-5"><div className="mb-4 flex items-start justify-between gap-4"><div><h3 className="font-mono text-xs font-semibold text-ink">{title}</h3><p className="mt-1 text-xs text-ink-muted">{description}</p></div><span className="rounded-[3px] border border-line-strong px-1.5 py-1 font-mono text-[9px] uppercase tracking-wider text-ink-subtle">shared</span></div>{children}</div>;
}
